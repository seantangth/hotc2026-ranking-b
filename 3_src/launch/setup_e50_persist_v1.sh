#!/usr/bin/env bash
# ============================================================================
# setup_e50_persist_v1.sh — E50 persist-K=8 獨立窗
#
# 判準：5_outputs/strategy_research_20260812/E50_PERSIST_K8_DESIGN_20260821.md
# 授權：E49 校正 KILL 後的獨立否證。零 LB。結束 terminate $INSTANCE_ID（D018）。
...
DEST="$GDRIVE/5_outputs/e50_persist_20260821"
...
die() {
  ...
  if command -v rclone >/dev/null 2>&1; then
    ...
    echo "DIED: $*" > /tmp/e50_died.txt
    rclone copyto /tmp/e50_died.txt "$DEST/DIED.txt" 2>/dev/null || true
  fi
  selfkill
  exit 1
}
...
PULL=$!
rclone copyto "$GDRIVE/3_src/peft/e50_probe_cuda.py" "$E50_DIR/e50_probe.py" || die "probe 缺"
rclone copyto "$GDRIVE/3_src/peft/e50_frames.json" "$E50_DIR/e50_frames.json" || die "frames json 缺"
...
p = hf_hub_download('1038lab/sam3','sam3.pt', local_dir='$E50_DIR/ckpt')
print('ckpt:', p)
" || die "權重下載失敗"
...
E = pathlib.Path(os.environ["E50_DIR"]); root = pathlib.Path.home()/"valfc"
seqs = list(json.load(open(E/"e50_frames.json"))["seqs"])
...
$PY3 "$E50_DIR/e50_probe.py" 2>&1 | tee "$OUT/e50_console.log"
RC=${PIPESTATUS[0]}
cp "$E50_DIR/e50_result.json" "$OUT/" 2>/dev/null || true
$PY3 - <<'PY'
import json, os
from pathlib import Path
p = Path(os.environ["E50_DIR"])/"e50_result.json"
if p.exists():
    d = json.loads(p.read_text())
    d.pop("records", None)
    Path.home().joinpath("e50_out/e50_summary.json").write_text(json.dumps(d, indent=1, ensure_ascii=False))
...
echo "✅ E50 完成"; tail -30 "$OUT/e50_console.log"
