#!/usr/bin/env bash
# ============================================================================
# setup_e49_persist_v1.sh — E49 persist-K 滯後拼接 canary
#
# 判準：5_outputs/strategy_research_20260812/E49_PERSIST_SPLICE_DESIGN_20260821.md
# 授權：Sean 08-21「趕緊去挖新政策、新編號」。零 LB。結束 terminate $INSTANCE_ID（D018）。
...
DEST="$GDRIVE/5_outputs/e49_persist_20260821"
...
die() {
  ...
  if command -v rclone >/dev/null 2>&1; then
    ...
    echo "DIED: $*" > /tmp/e49_died.txt
    rclone copyto /tmp/e49_died.txt "$DEST/DIED.txt" 2>/dev/null || true
  fi
  selfkill
  exit 1
}
...
PULL=$!
rclone copyto "$GDRIVE/3_src/peft/e49_probe_cuda.py" "$E49_DIR/e49_probe.py" || die "probe 缺"
rclone copyto "$GDRIVE/3_src/peft/e49_frames.json" "$E49_DIR/e49_frames.json" || die "frames json 缺"
...
p = hf_hub_download('1038lab/sam3','sam3.pt', local_dir='$E49_DIR/ckpt')
print('ckpt:', p)
" || die "權重下載失敗"
...
E = pathlib.Path(os.environ["E49_DIR"]); root = pathlib.Path.home()/"valfc"
seqs = list(json.load(open(E/"e49_frames.json"))["seqs"])
...
$PY3 "$E49_DIR/e49_probe.py" 2>&1 | tee "$OUT/e49_console.log"
RC=${PIPESTATUS[0]}
cp "$E49_DIR/e49_result.json" "$OUT/" 2>/dev/null || true
$PY3 - <<'PY'
import json, os
from pathlib import Path
p = Path(os.environ["E49_DIR"])/"e49_result.json"
if p.exists():
    d = json.loads(p.read_text())
    d.pop("records", None)
    Path.home().joinpath("e49_out/e49_summary.json").write_text(json.dumps(d, indent=1, ensure_ascii=False))
...
echo "✅ E49 完成"; tail -30 "$OUT/e49_console.log"
