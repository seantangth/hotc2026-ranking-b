#!/usr/bin/env bash
# ============================================================================
# setup_e18_samurai_sam3_v1.sh — E18：SAMURAI-on-SAM3（Kalman 運動先驗移植）
#
...
assert mean_gain > 0.10, f"❌ 閘門二 FAIL：病灶平均只改善 {mean_gain:+.4f}（需 >0.10）——Kalman 未攔住 identity switch，停工診斷"
print("✅ 閘門二過：Kalman 有效攔截 identity switch → 進全量")
GATE
...
# --- [7/7] 回傳 gDrive ------------------------------------------------------
echo "=== [7/7] 回傳 gDrive ==="
rclone copy "$OUT" "$GDRIVE/5_outputs/e18_samurai_sam3_20260806" --transfers 8
