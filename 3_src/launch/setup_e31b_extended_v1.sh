#!/usr/bin/env bash
# ============================================================================
# setup_e31b_extended_v1.sh — E31b 腿 B 延長驗證（16 epochs，單腿）
#
# 判準寫死在 3_src/peft/e31b_verdict_v1.py 檔頭（Sean 08-12 授權時寫在選項描述內）
# 目的：裁決 E31 腿 B 的 19.16%「下降」是真在學，還是大 LR 起點被推高後的擾動回歸
#
# 【單一變因】只跑腿 B（lr=1e-4）延長到 16 epochs；腿 A 沿用 E31 的 8ep 結果當基準
# 【主判準】末 2 epochs 中位數 vs 基準 0.5400：相對下降 >=10% ⇒ 真在學；<10% ⇒ 擾動回歸確認
...
DEST="$GDRIVE/5_outputs/e31b_extended_20260812"
SAM2REPO=~/sam2repo
SAM2_SHA=2b90b9f5ceec907a1c18123530e92e794ad901a4   # 與 Phase 1 同 SHA（D034 釘死）
PYT=~/trainenv/bin/python
EPOCHS="${EPOCHS:-16}"          # 16×152 = 2,432 步（延長驗證）
OUT=~/e31b_out; LOGS=~/e31b_logs
STAMP=~/e31b_timing.txt; : > "$STAMP"
mkdir -p "$OUT" "$LOGS"
...
die() {
  ...
  echo "DIED: $*" > /tmp/e31b_died.txt
  rclone copyto /tmp/e31b_died.txt "$DEST/DIED.txt" 2>/dev/null
  selfkill; exit 1
}
...
for f in freeze_spec_v1.py make_train_cfg_v2.py e31b_verdict_v1.py; do
  rclone copyto "$GDRIVE/3_src/peft/$f" ~/$f || die "拉 $f 失敗"
  [ -s ~/$f ] || die "$f 空檔"
done
# 判準基準 ＝ E31 腿 A（8 epochs）的末 2 epochs 中位數；拉不到則用檔內寫死的 0.5400
rclone copyto "$GDRIVE/5_outputs/e31_protocol_probe_20260812/logs/leg_A_console.log" \
  ~/e31_legA.log || echo "⚠️ 腿 A 基準 log 拉取失敗，判準改用寫死的 0.5400"
...
run_leg B 1.0e-4      # E31b 只跑腿 B（延長到 16 epochs）；腿 A 沿用 E31 的 8ep 結果當基準

# --- [5] 判準評定（判準寫在 e31b_verdict_v1.py 檔頭，此處僅執行）---------------
echo "=== [5] 判準評定 ==="
REF_ARG=""
[ -s ~/e31_legA.log ] && REF_ARG="--leg-a-ref $HOME/e31_legA.log"
# 用 trainenv 的 python（判準需要 numpy；系統 python3 不保證有）
$PYT ~/e31b_verdict_v1.py --leg-b-ext "$LOGS/leg_B_console.log" $REF_ARG \
  --json "$OUT/e31b_verdict.json" 2>&1 | tee "$OUT/E31B_VERDICT.txt"
mark E31B_DONE
cp "$STAMP" "$OUT/e31b_timing.txt"
rclone copy "$OUT" "$DEST/out" --transfers 8 || echo "⚠️ 最終回傳失敗"
rclone copy "$LOGS" "$DEST/logs" --include "*.log" --transfers 8

echo "✅ E31b 完成。判決："; cat "$OUT/E31B_VERDICT.txt" | tail -5
