#!/usr/bin/env bash
# ============================================================================
# setup_e28_cropwiden70_v1.sh — crop 面積門檻 0.40 → 0.60（**只新增序列，不動已驗證的 21 支**）
#
# 為何重開（時序證據，非翻案癖）：
...
GOUT="$GDRIVE/5_outputs/e28_cropwiden70_20260809"
PY3=~/sam3env/bin/python
INSTANCE_NAME="${INSTANCE_NAME:-hsot-e28}"
STAMP=~/e28_timing.txt; : > "$STAMP"
...
NEW_SEQS="vis-dashcam3 nir-car3 nir-dryleaf1"
...
(
  ...
  rclone copyto "$GDRIVE/5_outputs/submissions/sub_v008_e19cropzoom.csv" ~/v008.csv 2>/dev/null || true
...
$PY3 -m hsot.crop_rerun prep --frames-root "$FR" --base-csv ~/e02_test.csv \
  --envelope-extra ~/e15_test.csv --out-root ~/crop70 --meta ~/win70.json \
  --area-frac-max 0.70 --only-seqs $NEW_SEQS > ~/prep.log 2>&1 || die "prep 失敗（見 ~/prep.log）"
tail -14 ~/prep.log
NSEL=$($PY3 -c "import json;print(len(json.load(open('$HOME/win70.json'))))")
[ "$NSEL" -ge 3 ] || die "只選中 $NSEL 支（預期 3）——幾何與本機前測不符，停工"
rclone copyto ~/win70.json "$GOUT/win70.json"; rclone copyto ~/prep.log "$GOUT/prep.log"
mark PREP_DONE

# --- [3] 跑這 11 支 -----------------------------------------------------------
mark TRACK_START
ls ~/crop70 > ~/e28_list.txt
$PY3 ~/track_t1.py --frames-root ~/crop70 --seq-list ~/e28_list.txt \
  --out-dir ~/out_e28 --backend sam3 --sam3-ckpt ~/sam3.pt --sam3-eval \
  > ~/track.log 2>&1 || die "track 失敗（見 ~/track.log）"
tail -8 ~/track.log
rclone copy ~/out_e28/ "$GOUT/out_e28/"; rclone copyto ~/track.log "$GOUT/track.log"
mark TRACK_DONE

# --- [4] merge 進 v008 + 無 GT 災難檢查 --------------------------------------
mark MERGE_START
$PY3 -m hsot.crop_rerun merge --base-csv ~/v020.csv --crop-csv ~/out_e28/submission.csv \
  --meta ~/win70.json --out ~/sub_v021_cropwiden70.csv > ~/merge.log 2>&1 || die "merge 失敗"
tail -6 ~/merge.log

$PY3 - > ~/e28_check.txt 2>&1 <<'PYEOF'
import json, os
import numpy as np, pandas as pd
H = os.path.expanduser("~")
NEW = set(json.load(open(f"{H}/win70.json")).keys())
a = pd.read_csv(f"{H}/v020.csv"); b = pd.read_csv(f"{H}/sub_v021_cropwiden70.csv")
...
PYEOF
cat ~/e28_check.txt
rclone copyto ~/e28_check.txt "$GOUT/check.txt"
rclone copyto ~/sub_v021_cropwiden70.csv "$GDRIVE/5_outputs/submissions/sub_v021_cropwiden70.csv"
rclone copyto ~/sub_v021_cropwiden70.csv "$GOUT/sub_v021_cropwiden70.csv"
mark MERGE_DONE
