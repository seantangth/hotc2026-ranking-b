#!/usr/bin/env bash
# ============================================================================
# setup_e32_stage0_v1.sh — E32 Stage 0：SAM3 detector 對小目標的 recall 探針
#
...
DEST="$GDRIVE/5_outputs/e32_stage0_20260813"
...
PULL=$!
rclone copyto "$GDRIVE/3_src/peft/stage0_probe_cuda.py" "$E32_DIR/stage0_probe.py" || die "probe 腳本缺"
rclone copyto "$GDRIVE/3_src/peft/stage0_frames.json" "$E32_DIR/stage0_frames.json" || die "樣本清單缺"
...
SEQ=$($PY3 -c "import json;print(json.load(open('$E32_DIR/stage0_frames.json'))['seq'])")
SRC=$(find ~/valfc -maxdepth 3 -type d -name "$SEQ" -print -quit)
[ -n "$SRC" ] || die "找不到序列 $SEQ"
mkdir -p "$E32_DIR/frames" && ln -sfn "$SRC" "$E32_DIR/frames/$SEQ"
echo "幀就位：$(ls "$E32_DIR/frames/$SEQ" | wc -l) 張"

# --- [5] 探針（判準寫在 DESIGN 檔與腳本內，此處僅執行）--------------------------
$PY3 "$E32_DIR/stage0_probe.py" 2>&1 | tee "$OUT/stage0_console.log"
RC=${PIPESTATUS[0]}
cp "$E32_DIR/stage0_result.json" "$OUT/" 2>/dev/null
rclone copy "$OUT" "$DEST/out" --transfers 8 || echo "⚠️ 回傳失敗"
[ $RC -ne 0 ] && echo "⚠️ 探針非零退出 rc=$RC（log 已回傳，可本機重判）"

echo "✅ E32 Stage 0 完成"; tail -5 "$OUT/stage0_console.log"
