#!/usr/bin/env bash
# ============================================================================
# setup_e51_update_v1.sh — E51 同意才更新 exemplar
#
# 判準：5_outputs/strategy_research_20260812/E51_UPDATE_ON_AGREE_DESIGN_20260821.md
# 授權：Sean 08-21「所以呢？你也沒招了？」。零 LB。結束 terminate $INSTANCE_ID（D018）。
...
DEST="$GDRIVE/5_outputs/e51_update_20260821"
...
die() {
  ...
  if command -v rclone >/dev/null 2>&1; then
    ...
    echo "DIED: $*" > /tmp/e51_died.txt
    rclone copyto /tmp/e51_died.txt "$DEST/DIED.txt" 2>/dev/null || true
  fi
  selfkill
  exit 1
}
...
PULL=$!
rclone copyto "$GDRIVE/3_src/peft/e51_probe_cuda.py" "$E51_DIR/e51_probe.py" || die "probe 缺"
rclone copyto "$GDRIVE/3_src/peft/e51_frames.json" "$E51_DIR/e51_frames.json" || die "frames json 缺"
...
p = hf_hub_download('1038lab/sam3','sam3.pt', local_dir='$E51_DIR/ckpt')
print('ckpt:', p)
" || die "權重下載失敗"
...
E = pathlib.Path(os.environ["E51_DIR"]); root = pathlib.Path.home()/"valfc"
seqs = list(json.load(open(E/"e51_frames.json"))["seqs"])
...
$PY3 "$E51_DIR/e51_probe.py" 2>&1 | tee "$OUT/e51_console.log"
RC=${PIPESTATUS[0]}
cp "$E51_DIR/e51_result.json" "$OUT/" 2>/dev/null || true
$PY3 - <<'PY'
import json, os
from pathlib import Path
p = Path(os.environ["E51_DIR"])/"e51_result.json"
if p.exists():
    d = json.loads(p.read_text())
    d.pop("records", None)
    Path.home().joinpath("e51_out/e51_summary.json").write_text(json.dumps(d, indent=1, ensure_ascii=False))
...
echo "✅ E51 完成"; tail -30 "$OUT/e51_console.log"
