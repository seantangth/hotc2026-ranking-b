#!/usr/bin/env python3
"""E49 persist-K 滯後拼接探針。判準見 E49_PERSIST_SPLICE_DESIGN_20260821.md（執行前已寫死）。"""
import json, os, shutil, sys, warnings
from pathlib import Path
warnings.filterwarnings("ignore")

SP = Path(os.environ.get("E49_DIR", str(Path.home() / "e49")))
CFG = json.load(open(SP / "e49_frames.json"))
...
def apply_policy(rs, k=K, m=M, tau=TAU, agree=AGREE):
    rs = sorted(rs, key=lambda r: (r["window"], r["idx_in_win"]))
    by_win = {}
    for r in rs:
        by_win.setdefault(r["window"], []).append(r)
    fire = {}
    for wid, wr in by_win.items():
...
json.dump(res, open(SP / "e49_result.json", "w"), indent=1, ensure_ascii=False)
